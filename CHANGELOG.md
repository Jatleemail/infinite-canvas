...
+ [新增] 画布助手对话模式默认启用 Agent 模式：助手可主动调用工具操作画布（选中、添加文本节点、连线、删除、网格/水平/垂直布局、读取详情、改 prompt 等），单次最多 6 轮工具调用；模型不支持 tool calling 时关闭 Agent 开关即可降级为纯回答。
+ [新增] 画布助手新增"视频"和"音频"两种生成模式：直接在助手面板里描述需求即可生成视频/音频，结果带预览播放器并可一键插入画布为视频/音频节点；视频模式支持选中图片节点作为参考图。
+ [优化] 画布助手面板移除"未开发"标签，错误回复改为红色错误样式并支持一键重试；按 Ctrl/Cmd + I 可快速打开/收起助手。
+ [优化] 画布助手对话模式自动注入画布概览（节点类型分布、节点元数据摘要、连线、当前选中），无需选中节点也能基于全画布回答问题。
+ [新增] 云端渠道新增"默认音频模型"配置项，并支持 Vidu 协议语音合成（`/ent/v2/audio-tts`）；画布音频节点在 Vidu 音频模型下自动切换到 Vidu 内置音色下拉。
+ [新增] 云端渠道支持 Vidu 协议生图（viduq2），管理员可在后台新增 Vidu 渠道后将其设为默认生图模型，对前台调用透明。
+ [新增] 云端渠道支持 Vidu 协议视频生成：单参考图走 `/ent/v2/img2video`，多参考图走 `/ent/v2/multiframe`；前端按 Seedance 协议组装请求，后端做协议翻译并钳位 duration / resolution。
